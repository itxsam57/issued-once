...
async function lockTeeMediumBone(page: import('@playwright/test').Page) {
...
async function verifyContactAndShipping(page: import('@playwright/test').Page) {
...
test('captured referral discount can be replaced by a manual code and checkout uses only the replacement quote', async ({ page }, testInfo) => {
  await reachPhysicalForm(page);

  let referralCalls = 0;
  await page.route('**/api/referrals/apply', async (route) => {
    referralCalls += 1;
    const body = route.request().postDataJSON() as { quoteId: string; explicitCode?: string };

    if (referralCalls === 1) {
      expect(body.explicitCode).toBeUndefined();
      expect(body.quoteId.length).toBeGreaterThan(0);
      await route.fulfill({
        status: 200,
        contentType: 'application/json',
        body: JSON.stringify({
          quoteId: 'quote-link-e2e',
          grossAmountMinor: 3200,
          discountAmountMinor: 320,
          amountMinor: 2880,
          currency: 'USD',
    ...
    expect(body).toEqual({ quoteId: 'quote-link-e2e', explicitCode: 'creator-two' });
    await route.fulfill({
      status: 200,
      contentType: 'application/json',
      body: JSON.stringify({
        quoteId: 'quote-code-e2e',
        grossAmountMinor: 3200,
        discountAmountMinor: 640,
        amountMinor: 2560,
        currency: 'USD',
        expiresAt: '2026-08-21T13:00:00.000Z',
        applied: true,
        normalizedCode: 'CREATOR-TWO',
      }),
    });
  });

  await lockTeeMediumBone(page);
  await verifyContactAndShipping(page);
  ...
  await expect(page.getByText('CREATOR-TWO APPLIED')).toBeVisible();
  await expect(page.getByText('-$6.40')).toBeVisible();
  await expect(page.getByText('$25.60')).toBeVisible();
  await capture(page, `15-public-referral-commitment-${testInfo.project.name}`);

  await page.route('https://sandbox.api.getsafepay.com/checkout/**', async (route) => {
    await route.fulfill({
      status: 200,
      contentType: 'text/html',
      body: '<!doctype html><html><body><h1>SAFEPAY REFERRAL CHECKOUT TEST</h1></body></html>',
    });
  });
  await page.route('**/api/payments/create', async (route) => {
    const body = route.request().postDataJSON() as { quoteId: string };
    expect(body).toEqual({ quoteId: 'quote-code-e2e' });
    await route.fulfill({
      status: 200,
      contentType: 'application/json',
      body: JSON.stringify({
        paymentAttemptId: 'payment-referral-e2e',
        checkoutUrl: 'https://sandbox.api.getsafepay.com/checkout/pay?beacon=referral-e2e',
      }),
    });
  });

  await page.getByRole('button', { name: 'ISSUE MINE' }).click();
  await page.waitForURL('https://sandbox.api.getsafepay.com/checkout/pay?beacon=referral-e2e');
  await expect(page.getByRole('heading', { name: 'SAFEPAY REFERRAL CHECKOUT TEST' })).toBeVisible();
  expect(referralCalls).toBe(2);
});
