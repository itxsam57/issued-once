...
test('migration manifest points to repository tip and separates production-applied from deferred referral rollout', async () => {
  const [current, readme] = await Promise.all([
    readFile('db/migrations/CURRENT', 'utf8'),
    readFile('db/migrations/README.md', 'utf8'),
  ]);

  expect(current.trim()).toBe('0035_referral_private_payload_key_v2.sql');
