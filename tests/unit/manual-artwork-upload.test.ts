...
function png(width: number, height: number, ancillaryTextBytes = 0, alpha = 0): Buffer {
...
test('rejects a valid but fully opaque PNG before storing it', async () => {
  let stored = false;
  const service = new ManualArtworkUploadService(
    { getEffective: async () => ({ globalVersion: 1, override: null, policy: DEFAULT_DESIGN_POLICY }) },
    {
  ...
  await expect(service.upload({
    ...
    fileName: 'opaque.png',
    mimeType: 'image/png',
    bytes: png(1800, 2400, 12_000, 255),
    reason: 'opaque production asset should be blocked',
  })).rejects.toThrow(/transparent|alpha/i);
  expect(stored).toBe(false);
});
