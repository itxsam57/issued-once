...
function createHarness(input?: {
  ...
  const experiences = {
    findBySessionHash: vi.fn().mockResolvedValue(input?.source ?? sourceExperience),
  };
...
describe('RepeatOrderService', () => {
  ...
  it('creates a fresh child at QUESTION_1 and excludes all seven immediately previous question ids', async () => {
    ...
    const expectedExclusions = Object.fromEntries(
      harness.previous.map((question) => [question.family, question.questionId]),
    ...
    expect(result.questions.map((question) => question.questionId)).toEqual(
      harness.fresh.map((question) => question.questionId),
    );
  });
