...
export function SupportPanel() {
  ...
  async function refresh() {
    ...
    if (selected) setSelected((payload.items ?? []).find((item) => item.requestId === selected.requestId) ?? null);
  }
  useEffect(() => { void refresh().catch((cause) => setError(cause instanceof Error ? cause.message : 'Support unavailable')); }, [filter]);
  useEffect(() => { setReason(''); setMessage(null); setNote(''); setReply(''); }, [selected?.requestId]);
  ...
  return <div>
    <div className={styles.panelHead}>
      ...
      <select value={filter} onChange={(event) => setFilter(event.target.value as 'OPEN'|'CLOSED')}><option value="OPEN">OPEN</option><option value="CLOSED">CLOSED</option></select>
    </div>
    ...
    <div className={styles.ledgerLayout}>
      ...
      <section className={styles.detail}>{!selected ? <p>SELECT A CASE</p> : <>
        ...
        <p>Customer message remains encrypted until you explicitly reveal it.</p>
        ...
        <button disabled={working || !reason.trim()} type="button" onClick={() => void run(async () => {
