...
export function DesignerPanel() {
  ...
  async function refresh() {
    const next = await fetchDesignerQueue();
    setItems(next);
    if (selected) setSelected(next.find((item) => item.issueId === selected.issueId) ?? null);
  }
  async function loadCandidates(issueId: string) { setCandidates(await fetchCandidates(issueId)); }

  useEffect(() => {
    let alive = true;
    void fetchDesignerQueue()
  ...
  function choose(item: QueueItem) {
    ...
    setReason('');
    setError(null);
    void fetchCandidates(item.issueId)
      .then(setCandidates)
      .catch((cause) => setError(cause instanceof Error ? cause.message : 'Candidates unavailable'));
  }
