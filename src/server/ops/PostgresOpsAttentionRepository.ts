...
export type OpsAttentionKind = 'PAID_WITHOUT_ISSUE' | 'PAYMENT_EXCEPTION' | 'DESIGN_FAILED' | 'DESIGN_STUCK' | 'MANUFACTURING_FAILED' | 'NOTIFICATION_FAILED' | 'SUPPORT_AGING';
...
export class PostgresOpsAttentionRepository {
  ...
  async list(limit: number, now: Date): Promise<OpsAttentionItem[]> {
    const rows = await this.sql.query<Row>(
      ...
      LIMIT $1`,
      [Math.min(Math.max(Math.trunc(limit), 1), 100), now],
    );
    return rows.map((row) => ({
      kind: row.kind,
