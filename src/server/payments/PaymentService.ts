...
function safeOrigin(value: string): string {
  ...
  if (url.protocol !== 'https:' && url.hostname !== 'localhost') {
    throw new Error('Payment return origin must use HTTPS');
  }
  return url.origin;
}
...
export class PaymentService {
  ...
  async start(input: {
    ...
    const experience = await this.dependencies.experiences.findBySessionHash(
    ...
    if (!quote || quote.experienceId !== experience.id) {
    ...
    if (!shipping || shipping.contactId !== contact.id) {
      throw new Error('Shipping is required before payment');
    }

    const reusable = await this.dependencies.payments.findReusable(experience.id, quote.id);
    if (reusable?.checkoutUrl) {
      return { checkoutUrl: reusable.checkoutUrl, paymentAttemptId: reusable.id };
    }
    if (experience.stage === 'CHECKOUT_STARTED') {
      throw new Error('Checkout has already started');
    }
    ...
    const attempt: PaymentAttemptRecord = {
      ...
      shippingSnapshotId: shipping.id,
      provider: 'SAFEPAY',
    ...
    await this.dependencies.payments.create(attempt);
