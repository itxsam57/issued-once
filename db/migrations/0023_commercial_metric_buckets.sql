...
-- Backfill canonical sales and product mix for any pre-existing paid Issues.
INSERT INTO commercial_metric_buckets (bucket_day,metric_key,dimension_key,currency_scope,event_count,value_minor,value_seconds,updated_at)
SELECT (reserved_at AT TIME ZONE 'UTC')::date,'paid_order','all',currency,COUNT(*),0,0,NOW()
FROM issues WHERE payment_attempt_id IS NOT NULL AND currency IS NOT NULL
GROUP BY 1,currency
ON CONFLICT (bucket_day,metric_key,dimension_key,currency_scope) DO NOTHING;

INSERT INTO commercial_metric_buckets (bucket_day,metric_key,dimension_key,currency_scope,event_count,value_minor,value_seconds,updated_at)
SELECT (reserved_at AT TIME ZONE 'UTC')::date,'gross_paid','all',currency,COUNT(*),SUM(amount_minor),0,NOW()
FROM issues WHERE payment_attempt_id IS NOT NULL AND currency IS NOT NULL AND amount_minor IS NOT NULL
GROUP BY 1,currency
ON CONFLICT (bucket_day,metric_key,dimension_key,currency_scope) DO NOTHING;

INSERT INTO commercial_metric_buckets (bucket_day,metric_key,dimension_key,currency_scope,event_count,value_minor,value_seconds,updated_at)
SELECT (reserved_at AT TIME ZONE 'UTC')::date,'paid_order','object:'||object_type,currency,COUNT(*),0,0,NOW()
FROM issues WHERE payment_attempt_id IS NOT NULL AND currency IS NOT NULL AND object_type IS NOT NULL
GROUP BY 1,currency,object_type
ON CONFLICT (bucket_day,metric_key,dimension_key,currency_scope) DO NOTHING;

INSERT INTO commercial_metric_buckets (bucket_day,metric_key,dimension_key,currency_scope,event_count,value_minor,value_seconds,updated_at)
SELECT (reserved_at AT TIME ZONE 'UTC')::date,'paid_order','size:'||size_code,currency,COUNT(*),0,0,NOW()
FROM issues WHERE payment_attempt_id IS NOT NULL AND currency IS NOT NULL AND size_code IS NOT NULL
GROUP BY 1,currency,size_code
ON CONFLICT (bucket_day,metric_key,dimension_key,currency_scope) DO NOTHING;

INSERT INTO commercial_metric_buckets (bucket_day,metric_key,dimension_key,currency_scope,event_count,value_minor,value_seconds,updated_at)
SELECT (reserved_at AT TIME ZONE 'UTC')::date,'paid_order','color:'||color_code,currency,COUNT(*),0,0,NOW()
FROM issues WHERE payment_attempt_id IS NOT NULL AND currency IS NOT NULL AND color_code IS NOT NULL
GROUP BY 1,currency,color_code
ON CONFLICT (bucket_day,metric_key,dimension_key,currency_scope) DO NOTHING;

INSERT INTO commercial_metric_buckets (bucket_day,metric_key,dimension_key,currency_scope,event_count,value_minor,value_seconds,updated_at)
SELECT (issue.reserved_at AT TIME ZONE 'UTC')::date,'paid_order','country:'||shipping.country_code,issue.currency,COUNT(*),0,0,NOW()
FROM issues issue JOIN shipping_snapshots shipping ON shipping.id=issue.shipping_snapshot_id
WHERE issue.payment_attempt_id IS NOT NULL AND issue.currency IS NOT NULL
GROUP BY 1,issue.currency,shipping.country_code
ON CONFLICT (bucket_day,metric_key,dimension_key,currency_scope) DO NOTHING;

INSERT INTO commercial_metric_buckets (bucket_day,metric_key,dimension_key,currency_scope,event_count,value_minor,value_seconds,updated_at)
SELECT (issue.reserved_at AT TIME ZONE 'UTC')::date,'timing_start_to_paid','all','*',COUNT(*),0,
  SUM(EXTRACT(EPOCH FROM (issue.reserved_at-experience.created_at))),NOW()
FROM issues issue JOIN experiences experience ON experience.id=issue.experience_id
WHERE issue.payment_attempt_id IS NOT NULL
GROUP BY 1
ON CONFLICT (bucket_day,metric_key,dimension_key,currency_scope) DO NOTHING;

-- Backfill funnel evidence using the best persisted completion timestamp available.
INSERT INTO commercial_metric_buckets (bucket_day,metric_key,dimension_key,currency_scope,event_count,value_minor,value_seconds,updated_at)
SELECT (created_at AT TIME ZONE 'UTC')::date,'funnel_started','all','*',COUNT(*),0,0,NOW()
FROM experiences GROUP BY 1
ON CONFLICT (bucket_day,metric_key,dimension_key,currency_scope) DO NOTHING;

INSERT INTO commercial_metric_buckets (bucket_day,metric_key,dimension_key,currency_scope,event_count,value_minor,value_seconds,updated_at)
SELECT (updated_at AT TIME ZONE 'UTC')::date,'funnel_answered','all','*',COUNT(*),0,0,NOW()
FROM experiences WHERE stage NOT IN ('VISITOR','EXPERIENCE_STARTED','QUESTION_1','QUESTION_2','QUESTION_3','QUESTION_4','QUESTION_5','QUESTION_6','QUESTION_7')
GROUP BY 1
ON CONFLICT (bucket_day,metric_key,dimension_key,currency_scope) DO NOTHING;

INSERT INTO commercial_metric_buckets (bucket_day,metric_key,dimension_key,currency_scope,event_count,value_minor,value_seconds,updated_at)
SELECT (updated_at AT TIME ZONE 'UTC')::date,'funnel_physical','all','*',COUNT(*),0,0,NOW()
FROM experience_physical_selection WHERE color_code IS NOT NULL GROUP BY 1
ON CONFLICT (bucket_day,metric_key,dimension_key,currency_scope) DO NOTHING;

INSERT INTO commercial_metric_buckets (bucket_day,metric_key,dimension_key,currency_scope,event_count,value_minor,value_seconds,updated_at)
SELECT (verified_at AT TIME ZONE 'UTC')::date,'funnel_verified','all','*',COUNT(*),0,0,NOW()
FROM verified_contacts GROUP BY 1
ON CONFLICT (bucket_day,metric_key,dimension_key,currency_scope) DO NOTHING;

INSERT INTO commercial_metric_buckets (bucket_day,metric_key,dimension_key,currency_scope,event_count,value_minor,value_seconds,updated_at)
SELECT (created_at AT TIME ZONE 'UTC')::date,'funnel_shipping','all','*',COUNT(*),0,0,NOW()
FROM shipping_snapshots GROUP BY 1
ON CONFLICT (bucket_day,metric_key,dimension_key,currency_scope) DO NOTHING;

INSERT INTO commercial_metric_buckets (bucket_day,metric_key,dimension_key,currency_scope,event_count,value_minor,value_seconds,updated_at)
SELECT (created_at AT TIME ZONE 'UTC')::date,'funnel_checkout','all','*',COUNT(*),0,0,NOW()
FROM payment_attempts GROUP BY 1
ON CONFLICT (bucket_day,metric_key,dimension_key,currency_scope) DO NOTHING;

INSERT INTO commercial_metric_buckets (bucket_day,metric_key,dimension_key,currency_scope,event_count,value_minor,value_seconds,updated_at)
SELECT (updated_at AT TIME ZONE 'UTC')::date,'funnel_paid','all','*',COUNT(*),0,0,NOW()
FROM payment_attempts WHERE status IN ('PAID','REFUNDED') GROUP BY 1
ON CONFLICT (bucket_day,metric_key,dimension_key,currency_scope) DO NOTHING;

-- Backfill payment outcome event counts and refund money.
INSERT INTO commercial_metric_buckets (bucket_day,metric_key,dimension_key,currency_scope,event_count,value_minor,value_seconds,updated_at)
SELECT (updated_at AT TIME ZONE 'UTC')::date,
  CASE status WHEN 'FAILED' THEN 'payment_failed' WHEN 'EXCEPTION' THEN 'payment_exception' END,
  'all','*',COUNT(*),0,0,NOW()
FROM payment_attempts WHERE status IN ('FAILED','EXCEPTION')
GROUP BY 1,status
ON CONFLICT (bucket_day,metric_key,dimension_key,currency_scope) DO NOTHING;

INSERT INTO commercial_metric_buckets (bucket_day,metric_key,dimension_key,currency_scope,event_count,value_minor,value_seconds,updated_at)
SELECT (updated_at AT TIME ZONE 'UTC')::date,'refund','all',currency,COUNT(*),SUM(amount_minor),0,NOW()
FROM payment_attempts WHERE status='REFUNDED' GROUP BY 1,currency
ON CONFLICT (bucket_day,metric_key,dimension_key,currency_scope) DO NOTHING;

-- Backfill lifecycle timing from canonical Issue events.
INSERT INTO commercial_metric_buckets (bucket_day,metric_key,dimension_key,currency_scope,event_count,value_minor,value_seconds,updated_at)
SELECT (event.created_at AT TIME ZONE 'UTC')::date,'timing_paid_to_production','all','*',COUNT(*),0,
  SUM(EXTRACT(EPOCH FROM (event.created_at-issue.reserved_at))),NOW()
FROM issue_events event JOIN issues issue ON issue.id=event.issue_id
WHERE event.event_type='IN_PRODUCTION' GROUP BY 1
ON CONFLICT (bucket_day,metric_key,dimension_key,currency_scope) DO NOTHING;

INSERT INTO commercial_metric_buckets (bucket_day,metric_key,dimension_key,currency_scope,event_count,value_minor,value_seconds,updated_at)
SELECT (delivered.created_at AT TIME ZONE 'UTC')::date,'timing_production_to_delivered','all','*',COUNT(*),0,
  SUM(EXTRACT(EPOCH FROM (delivered.created_at-production.created_at))),NOW()
FROM issue_events delivered
JOIN LATERAL (
  SELECT created_at FROM issue_events prior
  WHERE prior.issue_id=delivered.issue_id AND prior.event_type='IN_PRODUCTION' AND prior.created_at<=delivered.created_at
  ORDER BY prior.created_at DESC LIMIT 1
) production ON true
WHERE delivered.event_type='DELIVERED' GROUP BY 1
ON CONFLICT (bucket_day,metric_key,dimension_key,currency_scope) DO NOTHING;

INSERT INTO commercial_metric_buckets (bucket_day,metric_key,dimension_key,currency_scope,event_count,value_minor,value_seconds,updated_at)
SELECT (support.created_at AT TIME ZONE 'UTC')::date,'support_request','all','*',COUNT(*),0,0,NOW()
FROM support_requests support GROUP BY 1
ON CONFLICT (bucket_day,metric_key,dimension_key,currency_scope) DO NOTHING;
