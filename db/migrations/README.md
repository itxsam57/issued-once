...
On 2026-08-21, migration `0029` was exercised on isolated Neon branch `proof-0029-creator-referrals-20260821` (`br-autumn-breeze-ax0afwq5`). Verification confirmed all seven referral tables, complete non-referral quote backfill across 108 inherited quotes with zero invariant violations, unique normalized referral codes, sub-100% percentage discounts, complete referral snapshot requirements, one conversion per payment attempt, reward lifecycle states, and one payout allocation per conversion. A positive synthetic referral snapshot froze gross 3200, discount 320, and final 2880. The production/default branch was independently checked and still had no referral tables or referral quote columns. The temporary proof branch was deleted after verification. Production remains verified only through `0028`; `0029` is not applied there.
