...
| CR-22 | Refund truth is provider-derived and an owner has a documented operational resolution path | CODE_READY | full-refund runbook/reconciliation workflow is integrated and exact-head green; controlled deployed Safepay full-refund/reconciliation proof still required |
...
### CR-22 — provider-derived refund operations and owner reconciliation

- Feature branch: `feat/cr-22-refund-reconciliation`; integration PR: #68; full-refund runbook hardening PR: #69.
- Final feature head: `3cf79dc4b8cacd3645c05dce2d503647245f1461`.
- TDD RED evidence: CI run `33497416050` failed only the two new PaymentService refund-reconciliation tests because `reconcileRefund` did not exist while 230 existing suites / 683 existing tests stayed green. Expanded RED CI run `33497754475` failed exactly four new assertions: the missing PaymentService reconciliation seam and missing Safepay full-refund verifier; all 230 existing suites / 683 existing tests remained green.
- Final feature CI: run `33498342341` PASS — unit tests, typecheck, lint and production build.
- Final feature Browser QA: run `33498342363` PASS.
- Initial integration merge: `2daff7d1c4706e0dffed7c64947f27b03e8b9d44`.
- Initial post-merge CI: run `33498686839` PASS — unit tests, typecheck, lint and production build; Browser QA run `33498686863` PASS.
- Final hardening integration head: `6b64b3b000cff18db2ac27b8c5494b0c72670211`.
- Final hardening CI: run `33499009895` PASS — unit tests, typecheck, lint and production build.
- Final hardening Browser QA: run `33499010036` PASS.
- Operational contract: ISSUED ONCE does not expose a browser action that initiates or fabricates financial refund truth. The owner initiates the **full refund** in Safepay Dashboard; signed Safepay refund webhook remains the automatic production truth source.
- Fallback reconciliation: authenticated Owner OS requires exact `VERIFY SAFEPAY <Issue Code>` confirmation, resolves the stored tracker and stored original money server-side, and can reconcile only provider-confirmed full refund state. Browser-supplied amount/currency/provider-reference fields cannot become authority.
- Provider boundary: full refund verification requires `TRACKER_REFUNDED` for the exact stored amount/currency and explicitly does not treat `TRACKER_PARTIAL_REFUND` as full-refund truth.
- Lifecycle/idempotency: webhook and owner reconciliation use the same refund finalizer, preserving guarded PAID -> REFUNDED transition, Issue quarantine, referral reversal/notification side effects and duplicate/replay safety.
- Remaining evidence: after the exact release is deployed, perform one explicitly authorized controlled full Safepay refund and prove signed webhook or Owner OS provider reconciliation updates exactly the intended Issue/payment with no cross-Issue effects.
- Production deployment/environment/provider mutation during code cycle: none; no real Safepay charge/refund was created for QA.
- Master-plan state: `CODE_READY`; not `DONE` until controlled deployed provider proof exists.
...
**Code-side root cause resolved and full-refund runbook hardened at integration `6b64b3b000cff18db2ac27b8c5494b0c72670211`; controlled provider proof remains.**

Refund initiation remains in Safepay Dashboard rather than an application browser action and the Owner OS runbook now explicitly instructs a **full refund**. Signed Safepay webhook remains the automatic truth source. Owner OS provides a separate provider-truth fallback that requires an authenticated exact Issue confirmation, loads the stored tracker/money authority server-side, accepts only a full `TRACKER_REFUNDED` result, and shares the same idempotent refund finalizer used by webhook processing. A partial refund never becomes full-refund application truth.

Remaining completion target: on the deployed exact release, execute one explicitly authorized controlled full provider refund and prove the intended payment/Issue/referral lifecycle converges without duplicate or cross-Issue effects.
...
9. `CR-22` refund operations/runbook truth — **code-ready and integrated/hardened at `6b64b3b000cff18db2ac27b8c5494b0c72670211`; controlled deployed provider proof pending**.
10. **NEXT OWNER-GATED STEP:** approve/preflight/apply production `0036`, deploy exact verified integration head `6b64b3b000cff18db2ac27b8c5494b0c72670211` while preserving production variables, then run `CR-27` live boundary/security proof.
